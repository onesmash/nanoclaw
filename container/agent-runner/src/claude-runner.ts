/**
 * Claude Agent Runner
 * Receives ContainerInput via stdin, runs Claude Agent SDK, outputs ContainerOutput via stdout.
 */
import fs from 'fs';
...
function log(message: string): void {
  console.error(`[claude-runner] ${message}`);
}
...
async function runQuery(
  ...
  if (ctx.extraDirs.length > 0) {
    log(`Additional directories: ${ctx.extraDirs.join(', ')}`);
  }
  ...
  for await (const message of query({
    ...
    if (message.type === 'assistant') {
      const textContent = (message as { message?: { content?: Array<{ type: string; text?: string }> } }).message?.content
        ...
        .join('') ?? '';
      if (textContent) {
        log(`Assistant text (${textContent.length} chars): ${textContent.slice(0, 100)}`);
        writeOutput({ status: 'success', result: textContent, newSessionId });
      }
    }
    ...
    if (message.type === 'result') {
      ...
      if (message.subtype !== 'success') {
        writeOutput({
          status: 'error',
          result: null,
          error: textResult || `Agent exited with subtype: ${message.subtype}`,
          newSessionId,
        });
      } else {
